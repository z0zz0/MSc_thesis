...
such as is done with Instance-level meta normalization but batch-wise rather than instance-wise.

Forward Statistics Capture
Batch Normalization
"""
## forward_statistics_catcher_BN (short fsc)
class fsc_BN3d(nn.Module):
    def __init__(self, channels, embed_size, eps=1e-5):
        ...
        ## register_buffer send e.g. "running_mean" to the GPU device along with the module but doesn't add to the graph
        self.register_buffer("running_mean", torch.zeros(channels))
        self.register_buffer("running_var", torch.zeros(channels))
        
        ## nn.Parameter adds variables to the graph
        self.weight_bias = nn.Parameter(torch.ones(1, channels, 1, 1, 1))
        self.bias_bias = nn.Parameter(torch.zeros(1, channels, 1, 1, 1))
